...
| `src/hooks/useHomeAssistant.js` | Full HA entity fetch — temp sensors, humidity, lights, scenes, battery sensors, system stats, OBD entities (`sensor.chomp_fuel_level`, `sensor.chomp_engine_coolant_temperature`, `sensor.chomp_voltage_obd_adapter`, `sensor.chomp_distance_to_empty`, `sensor.chomp_distance_travelled`, `sensor.chomp_ambient_air_temperature`, `sensor.chomp_engine_rpm`). Authenticated via `plaintextToken`. Triggers `requestUnlock` if token locked. ⚠ OBD entity IDs are provisional — confirm against HA Dev Tools. |
...
| Threat headline injection on Home (Slice 2b) | `threats[]` is on AppContext but `OnTripHome`/`PreTripHome` don't render any threat alerts yet. Separate slice. |
