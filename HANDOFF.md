...
- **Slice 2c — PreTripHome + PostTripHome weather/threat additions** ✓ smoke-tested prod 2026-05-26 — PreTripHome: ThreatHeadline + SnoozedPill wired (surface='home' filter, priority desc top 1, sessionStorage dismiss shared with OnTripHome via `useDismissedThreats`). PreTripHome + PostTripHome: compact weather row from `AppContext.weather`. PostTripHome intentionally has no threat headline (unloading phase is calm by design per mockup). PreTripHome intel section's Weather IntelRow removed — superseded by the compact live row. `SnoozedPill` extracted to module-scope helper in `HomePage.jsx` (consumed by `OnTripHome` + `PreTripHome`). Threat scoping remains current-GPS; waypoint-scoped pre-trip threats stay deferred. No new hooks, deps, or tokens.
...
- **Slice 2d.1 — ThreatDetailSheet** — New `src/components/ThreatDetailSheet.jsx`: slide-up bottom sheet (200ms ease-out translateY), `createPortal` to `document.body` (z-index 200), ESC handler, backdrop-tap close, safe-area-inset-bottom padding. Content: severity-tinted header + icon+headline, distance (mi) + 8-point cardinal bearing, trajectory, detail prose, NWS DETAILS/INSTRUCTIONS text blocks (pre-wrap, parsed from `wx.alerts.properties.description`), source attribution, `ActionButtons` mirror. `ThreatHeadline.jsx` refactored: `SEVERITY`, `ThreatIcon`, `ActionButtons` now named exports; header+detail prose wrapped in `role="button" tabIndex={0}` tap zone with `onOpenDetail` prop (cursor:pointer when prop present, noop-safe via `?.`). `OnTripHome` + `PreTripHome` in `HomePage.jsx` wired: `const [openThreat, setOpenThreat] = useState(null)`, `onOpenDetail={() => setOpenThreat(headlineThreat)}` on ThreatHeadline, `<ThreatDetailSheet threat={openThreat} onClose={() => setOpenThreat(null)} />` rendered unconditionally (returns null when threat is null). No new deps or tokens.
...
| Pre-storm checklist sheet | `weather_alert` action button removed in Slice 2d.2 — no checklist destination exists. When built, restore button in `ActionButtons` routing to the new sheet. |
| Escape routes section on SafetyPage | `wildfire` "Escape routes" button removed in Slice 2d.2. `Conditions` row "Escape route" is seed-only. When real route data exists (trip waypoints + bailouts), restore button + add a dedicated `id=safety-escape-routes` section. |
