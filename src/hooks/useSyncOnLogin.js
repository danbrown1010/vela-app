...
import { getGearItems, getPendingDeletes, removePendingDelete, clearPendingSaves, saveGearItem, clearGearPendingSync } from '../utils/gearStorage'
...
export async function runLoginSync(user, setSyncStatus, onError) {
  ...
  try {
    ...
    // Clear all IDB pending flags — everything is now server-authoritative
    await clearGearPendingSync()
