...
import { getPendingTrackDeletes, removePendingTrackDelete } from '../utils/trackStorage'
...
import { setLastSyncTime, emitSyncChanged } from '../utils/pendingSync'

const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i

// One-time migration: silently drop pending-delete entries whose IDs are
// not valid UUIDs (e.g. mock-*) or that pre-date proper sync wiring. These
// rows never existed server-side so no Supabase call is needed.
// Also sweeps vela-pending-trip-saves for any lingering mock trip saves.
// TODO(cleanup): remove after Dec 2026 — one-time mock data purge
function cleanupPhantomDeletes() {
  const isPhantom = id => id.startsWith('mock-') || !UUID_RE.test(id)
  let cleaned = 0

  for (const id of getPendingDeletes().filter(isPhantom)) { removePendingDelete(id); cleaned++ }
  for (const id of getPendingTripDeletes().filter(isPhantom)) { removePendingTripDelete(id); cleaned++ }
  for (const id of getPendingTrackDeletes().filter(isPhantom)) { removePendingTrackDelete(id); cleaned++ }
  for (const id of Object.keys(getPendingTripSaves()).filter(isPhantom)) { removePendingTripSave(id); cleaned++ }

  if (cleaned > 0) emitSyncChanged()
}

// Returns { succeeded, failed, errors, offline, unexpected }
export async function runLoginSync(user, setSyncStatus, onError) {
  cleanupPhantomDeletes()
